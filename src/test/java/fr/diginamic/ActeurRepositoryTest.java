...
public class ActeurRepositoryTest {
	...
	public void testExtraireActeursParPays() {
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a JOIN a.roles r JOIN r.film f JOIN f.pays fp WHERE fp.nom = 'France'", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireParRealisateurEntreAnnee() {
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a JOIN a.roles r JOIN r.film f JOIN f.realisateurs rd WHERE rd.identite = 'Ridley Scott' AND f.annee >= 2010 AND f.annee <= 2020", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
